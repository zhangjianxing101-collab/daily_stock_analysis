...
def test_market_style_accepts_high_coverage_positive_market_cap_evidence() -> None:
    market = _market_payload(dataset(pd.DataFrame({
        "change_pct": [1.0, -1.0, 0.0, 2.0],
        "amount": [10.0, 20.0, 30.0, 40.0],
    ...
    assert market["市场风格"] == "大盘占优"
    assert market["市值字段覆盖记录"] == 3


def test_market_style_rejects_low_market_cap_coverage() -> None:
    market = _market_payload(dataset(pd.DataFrame({
        "change_pct": [1.0, -1.0, 0.0, 2.0],
        "amount": [10.0, 20.0, 30.0, 40.0],
        "total_mv": [10.0, None, None, 40.0],
    })), authoritative=True)

    assert market["市场风格"] == "不可用"
    assert "市值字段覆盖记录" not in market
