...
- [修复] 协同收盘日报读取腾讯行情总市值并在覆盖率达到 70% 时计算市场风格，同时将候选 AI 隔离进程上限调整为 10 分钟，仍拒绝占位分析与超时报告发送。

- [修复] 历史收盘报告测试邮件补发增加核心模块与市场风格校验，拒绝已有章节但实际不满足交付条件的预览工件。

- [新功能] 协同日报使用经校验的总市值分组补充大小盘风格与组内平均涨跌幅，并按报告交易日接入东方财富涨停、跌停股池；缺失时明确降级，不以涨跌幅阈值估算。

- [修复] 协同收盘日报先获取同花顺板块及成分股，再将真实强势行业归属用于技术候选排序、板块龙头精选和有限 AI 分析，避免板块数据已存在但候选仍无板块证据。

- [新功能] 协同收盘日报新增独立校验的行业与概念强弱榜、轮动/持续性/拥挤度、候选板块上下文及决策摘要；通过最多五个更早的正式收盘产物衔接脱敏板块状态，缺失或部分数据明确降级且不伪造结论。

- [修复] 协同日报隔离明确无报价且无成交的记录，保留异常值与身份校验并拒绝跨交易日分页；持仓无法估值时暂停新增仓位测算；选股前通过独立时间和价格核对补齐腾讯名称、量比及换手率，公开显示字段覆盖情况，并为无邮件预览增加脱敏数据质量统计。

- [修复] 协同日报兼容既有 GitHub `THS` 行情密钥名称，优先使用 `THS_API_KEY`；黄金历史请求限定到最后已收盘交易日；行情按响应接收时间校验，报告按实际生成时间记录，保持跨日、未来数据和时效限制，禁止上传失效报告，并增加脱敏数据失败代码及接口连接分类。
...
- [修复] 协同 A 股日报在 XSHG 内置交易日历覆盖不足时改用经过双向覆盖范围与格式校验的 AkShare 交易日数据，按上海本地日期刷新缓存，并限制备用日历子进程最多运行 30 秒；缺失、过期、超时或异常数据继续安全失败且不发邮件，不再把未知日期误判为休市。
