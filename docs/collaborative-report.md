...
Use a production manual run only after verifying the secrets and variables. A normal sent report creates `sent-<report-key>`; a later fresh GitHub job uses that durable marker as the cross-run duplicate guard and passes `--already-sent` to the runner as an external completed identity. Test email bypasses this marker lookup. Within a single run or a locally durable output directory, the runner's local delivery ledger is the delivery state machine; it is not cross-run state on GitHub-hosted runners. `in_doubt` and reconcile-required outcomes intentionally do not send again and do not create a sent marker. Reconcile the delivery state before a later production retry.
