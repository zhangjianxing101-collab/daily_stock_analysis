...
Use **Run workflow** for a manual `premarket` or `postmarket` report. For a manual test email, select `premarket`, set `force` to true when outside its normal window, and set `test_email` to true. A test email is marked as a test and never creates a delivery marker.

Use a production manual run only after verifying the secrets and variables. A normal sent report creates `sent-<report-key>`; a repeat production run checks that marker as a fast external duplicate hint, while the runner's local delivery ledger remains the source of truth. `in_doubt` and reconcile-required outcomes intentionally do not send again and do not create a sent marker. Reconcile the delivery state before a later production retry.

## Privacy and retention

The private `report-<report-key>` artifact can contain portfolio analysis and is available only to repository users with Actions artifact access. The diagnostic artifact is redacted and contains only report state, module statuses, source timestamps, and warning codes. Workflow logs and the concise summary do not print the report HTML, report text, mailbox values, portfolio values, credentials, or raw model responses. All artifacts expire after seven days.
